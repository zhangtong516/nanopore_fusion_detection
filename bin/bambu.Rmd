---
title: "Detecting fusion isoform with bambu"
output:
  html_notebook:
    toc: yes
params:
  ...
  bam: "sample1.bam"
  jaffaresults: "jaffaresults.tsv"
  gtf: "gencode.v49.primary_assembly.annotation.gtf"
  refgenomefasta: "GRCh38.primary_assembly.genome.fa"
...
getFusionAnnotations <- function(fusionGeneNames, anno.file,ensemblAnnotations.genes,ensemblAnnotations.txs,fusionTx){
    annotationRanges <- bambu::prepareAnnotations(anno.file)
    fusionAnnotation <- lapply(fusionGeneNames, function(s){
      print(s)
      if((!s %in%  names(fusionTx)) || is.null(fusionTx[[s]]) || (is.character(fusionTx[[s]]) && fusionTx[[s]] == "")) { return(NULL) }  ## if the fusion gene is not in the fusionTx, then return NULL
      ...
      tmp_range <- do.call("c",lapply(genevec, function(g){
        ...
        # print(length(txid))
        res <- do.call("c",lapply(txid, function(t){
          print(t)
          # tmp <- annotationRanges[t]
          tmp <- annotationRanges[t]
          if(!(t %in% names(fusionTx[[s]]))) { 
            return(NULL)
          }
          ...
          # if negative strand
          strand_val <- as.character(unique(strand(tmp[[1]])))[1]
  ...
  fusionAnnotation <- GRangesList(fusionAnnotation)
...
getFusionTxRange <- function(fusionGeneNames, ensemblAnnotations.genes, exonsByGene, ensemblAnnotations.txs, exonsByTx, jaffa_results_table){
  ...
  geneid2 <- ensemblAnnotations.genes[hgnc_symbol==genevec[2]]$ensembl_gene_id
  if(length(geneid) == 0) {
    return(NULL) 
  }
  if(length(geneid2) == 0) {
  ...
  tmp_range <- lapply(seq_along(genevec), function(g){
    ...
    new_range <- lapply(txid, function(t){
      ...
      if(g==1){
        scoreNum = ifelse(!is.na(strand.info) && strand.info=="-", length_g_tmp-jaffa_results_table[fusion.genes == tmp]$base1+1,
                          jaffa_results_table[fusion.genes == tmp]$base1-min_start+1)
      }else{
        scoreNum = ifelse(!is.na(strand.info) && strand.info=="-",length_g_tmp-jaffa_results_table[fusion.genes == tmp]$base2+1+length_g1,
                          jaffa_results_table[fusion.genes == tmp]$base2-min_start+1+length_g1)
      }
      if(!is.na(strand.info) && strand.info=="-"){
...
getFusionGeneRange <- function(fusionGeneNames, ensemblAnnotations.genes,exonsByGene, jaffa_results_table){
  ...
  tmp_range <- lapply(seq_along(genevec), function(g){
    ...
    min_start <- min(start(tmp_range)) 
    length_g_tmp <- max(end(tmp_range)) 
    strand.info <- as.character(unique(strand(tmp_range)))[1]
    if(g==1){
      scoreNum = ifelse(!is.na(strand.info) && strand.info=="-", length_g_tmp-jaffa_results_table[fusion.genes == tmp]$base1+1,
                        jaffa_results_table[fusion.genes == tmp]$base1-min_start+1)
    }else{
...
getFusionBreakpoints <- function(fusionGeneNames, ensemblAnnotations.genes, exonsByGene, jaffa_results_table){
...
The pipeline will be presented with three sections:
...
jaffa_results_table <- fread(params$jaffaresults, check.names = TRUE)
fusionGeneNames <- unique(jaffa_results_table[, get("fusion.genes")])

```
This chunk of code is to load the annotation information for genes from Gencode V42. 

```{r prepare_annotation_data, echo=F}

txdb <- txdbmaker::makeTxDbFromGFF(params$gtf)
exonsByGene <- exonsBy(txdb, 'gene')
exonsByTx <- exonsBy(txdb,"tx", use.names = TRUE)

gtf_data <- import(params$gtf) 
gtf_df <- as.data.frame(gtf_data) 
ensemblAnnotations.genes <- gtf_df %>%
  filter(type == "gene") %>%
  dplyr::select(
    ensembl_gene_id = gene_id,
    hgnc_symbol = gene_name,
    gene_biotype = gene_type,
    # description = "", # Note: description may require a specific GENCODE version or additional mapping
    chromosome_name = seqnames,
    start_position = start,
    end_position = end,
    strand
  )
#ensemblAnnotations.genes$ensembl_gene_id <- gsub("\\..*","", ensemblAnnotations.genes$ensembl_gene_id)
rownames(ensemblAnnotations.genes) = ensemblAnnotations.genes$ensembl_gene_id 
...
ensemblAnnotations.txs <- gtf_df %>%
  filter(type == "transcript") %>%
  left_join(tx_stats, by = "transcript_id") %>%
  dplyr::select(
    ensembl_transcript_id = transcript_id,
    ensembl_gene_id = gene_id,
    hgnc_symbol = gene_name,
    gene_biotype = gene_type, 
    # description = gene_description, # Optional: only if present in GTF
    chromosome_name = seqnames,
    start_position = start,
    end_position = end,
    strand,
...
#ensemblAnnotations.txs$ensembl_gene_id <- gsub("\\..*","", ensemblAnnotations.txs$ensembl_gene_id)
#ensemblAnnotations.txs$ensembl_transcript_id <- gsub("\\..*","", ensemblAnnotations.txs$ensembl_transcript_id)
rownames(ensemblAnnotations.txs) = ensemblAnnotations.txs$ensembl_transcript_id 
ensemblAnnotations.txs <- data.table(ensemblAnnotations.txs, keep.rownames = TRUE)
```

This chunk of code is to load human chromosome sequences. 

```{r prepare_genome_fasta, echo=F}
library(Biostrings)
cat('Load transcript sequence information')
geneSeq <- readDNAStringSet(file=params$refgenomefasta)
names(geneSeq) <- sapply(strsplit(names(geneSeq), " "), `[`, 1)
...
fusionGeneSequence = rbindlist(lapply(fusionGeneNames, function(s){    
    ...
    fusionseq = paste(unlist(lapply(seq_along(genevec), function(g){
        ...
        if(length(geneid) > 1 ) {geneid = geneid[1]}
        tmp_range <- exonsByGene[geneid][[1]]
        GenomeInfoDb::seqlevelsStyle(tmp_range) = "UCSC"
...
```{r fusionGeneFasta_file}
# fusionFastaFile = paste0(params$samplename, "_fusionGene_sequences.fa" )
# fasta_sequences = Biostrings::DNAStringSet(fusionGeneSequence$fusion_seq)
# names(fasta_sequences) = fusionGeneSequence$fusion_gene
# Biostrings::writeXStringSet(fasta_sequences, file = fusionFastaFile) 

fusionFastaFile = "fusionGene.fasta"
sink("fusionGene.fasta")
noprint <- lapply(seq_along(fusionGeneNames), function(s){
    cat(paste0(">",fusionGeneNames[s]), " \n")
    cat(fusionGeneSequence[s, fusion_seq], " \n")
})
sink()
```
...
```{r}
anno.file <- params$gtf # "hg38_sequins_SIRV_ERCCs_longSIRVs_corrected.gtf"
...
```{r}
system(paste0("samtools view -bhL fusion.bed ", params$bam, " | samtools fastq - > ", params$samplename, "_fusion.fastq")) 
```

This chunk of code will align the fusion fastq to fusion chromosomes

```{r}
system(paste0("minimap2 -ax splice -G2200k -N 5 --sam-hit-only -t 20 fusionGene.fasta ", params$samplename, "_fusion.fastq | samtools sort - -O bam -o ", params$samplename, "_fusion.bam"))
system(paste0("samtools index ", params$samplename, "_fusion.bam"))
``` 
...
fusionBamFile = paste0(params$samplename, "_fusion.bam") 
seFusion <- bambu(reads = fusionBamFile,rcOutDir = rcSaveDir, annotations = fusionAnnotations, genome = fusionFastaFile, 
                                 fusionMode = F, stranded = FALSE, ncore = 4, yieldSize = 1000000, verbose = FALSE)
saveRDS(seFusion, file = paste0(params$samplename, "_seFusion.rds")) 
```
...
reads <-  readGAlignments(file=params$bam, 
